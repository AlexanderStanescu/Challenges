/**
 * 
 * "You forgot to give Professor Boolean's favorite rabbit specimen a name? You know how picky the professor is! Only particular names will do! Fix this immediately, before you're... eliminated!"

	Luckily, your minion friend has already come up with a list of possible names, and we all know that the professor has always had a thing for names with lots of letters near the 'tail end' of the alphabet, so to speak. 
	
	You realize that if you assign the value 1 to the letter A, 2 to B, and so on up to 26 for Z, and add up the values for all of the letters, the names with the highest total values will be the professor's favorites. 
	
	For example, the name Annie has value 1 + 14 + 14 + 9 + 5 = 43, while the name Earz, though shorter, has value 5 + 1 + 18 + 26 = 50.

	If two names have the same value, Professor Boolean prefers the lexicographically larger name. For example, if the names were AL (value 13) and CJ (value 13), he prefers CJ.

	Write a function answer(names) which takes a list of names and returns the list sorted in descending order of how much the professor likes them.

	There will be at least 1 and no more than 1000 names. Each name will consist only of lower case letters. The length of each name will be at least 1 and no more than 8.

	Test cases:

	Inputs:

	(string list) names = ["annie", "bonnie", "liz"]
	Output:

	(string list) ["bonnie", "liz", "annie"]
	Inputs:

	(string list) names = ["abcdefg", "vi"]
	Output:

	(string list) ["vi", "abcdefg"]
 *
 */

public class OrderNames {	
	static class TreeNode {
		String name;
		int value;
		TreeNode left;
		TreeNode right;
		
		public TreeNode(String name, int value) {
			this.name = name;
			this.value = value;
		}
	}
	
	static class BinarySearchTree {
		TreeNode root;
		
		public void insert(String name, int value) {
			root = insert(root, new TreeNode(name, value));
		}
		
		private TreeNode insert(TreeNode parent, TreeNode node) {						
			if (parent == null) {
				return node;
			} else if (node.value > parent.value) {
				parent.left = insert(parent.left, node);
			} else if (node.value < parent.value) {
				parent.right = insert(parent.right, node);
			} else if (node.name.compareTo(parent.name) > 0) {
				parent.left = insert(parent.left, node);
			} else if (node.name.compareTo(parent.name) < 0) {
				parent.right = insert(parent.right, node);
			}		
			
			return parent;
		}
				
		public int inOrderTraversal(int loc, String[] arr, TreeNode node) {
			if (node == null)
				node = root;
			
			if (node.left != null) {
				loc = inOrderTraversal(loc, arr, node.left);
			}
			
			arr[loc++] = node.name;
			
			if (node.right != null) {
				loc = inOrderTraversal(loc, arr, node.right);
			}
			
			return loc;
		}
		
		public TreeNode getRoot() { return root; }
	}

	public static void main(String[] args) {
		String[] names = {"annie", "bonnie", "liz"};
		
		for (String s : answer(names)) {
			System.out.println(s);
		}
	}
	
	public static String[] answer(String[] names) {
		int length = names.length;
		char[] name = null;
		int value = 0;
		BinarySearchTree bst = new BinarySearchTree();
		String[] answer = new String[names.length];
		
		for (int i = 0; i < length; i++) {
			name = names[i].toCharArray();
			
			for (int j = 0; j < name.length; j++)
				value += (name[j] - 96);
			
			bst.insert(new String(name), value);
			
			value = 0;
		}
		
		bst.inOrderTraversal(0, answer, bst.getRoot());
		
		return answer;
	}
}